...
public class HTTPCounterDao implements CounterDao {

	private final String servletUrl;
	private final String baseUrl;
	
	public HTTPCounterDao(final String serverUrl) {
		...
		url.append("Data");
		url.append("?").append(DataServlet.PARAM_DATATYPE).append("=").append(DataServlet.DATATYPE_COUNTER);
	...
	public List<String> findAll(final String unitName) {
		...
		try {
			...
			urlString.append("&unitName=").append(unitName);
	...
	public Image load(String unitName) throws Exception {
		List<String> filenames = findAll(unitName);

		if ((filenames != null) && (filenames.size() > 0)) {
			return ImageIO.read(URLEncoder.encode(getBaseUrl() + filenames.get(0)));
		} else {
			return null;
		}
	}
